...
class Invoice_model extends CORE_Model {
    ...
    function get_invoice_items($salesfromdate,$salestodate){
        $sql="SELECT * FROM pos_invoice
              LEFT JOIN pos_invoice_items ON
              pos_invoice_items.pos_invoice_id=pos_invoice.pos_invoice_id
              LEFT JOIN products ON
              products.product_id=pos_invoice_items.product_id
              WHERE pos_invoice.transaction_date BETWEEN '".$salesfromdate."' AND '".$salestodate."'
              ";
